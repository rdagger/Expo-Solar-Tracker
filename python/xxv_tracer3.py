...
#
# Modified by rdagger for Python3 (12-25-2019)
...
class Result(object):
    ...
    def __str__(self):
        return "%s{%s}" % (self.__class__.__name__, ", ".join(
            map(lambda a: "%s: %s" % (a, getattr(self, a)), self.props)))


class QueryResult(Result):
    """The result of a query command."""
    props = ['batt_voltage', 'pv_voltage', 'load_amps',
             'batt_overdischarge_voltage', 'batt_full_voltage', 'load_on',
             'load_overload', 'load_short', 'batt_overload',
             'batt_overdischarge', 'batt_full', 'batt_charging', 'batt_temp',
             'charge_current']

    def decode(self, data):
        """Decodes the query result, storing results as fields"""
        if len(data) < 23:
            print('Not enough data. Need 23 bytes, got {0}'.format(len(data)))
...
class TracerSerial(object):
    ...
    def from_bytes(self, data):
        """Given bytes from serial port, returns appropriate command result"""
        if data[0:6] != self.sync_header:
            raise Exception("Invalid sync header")
        if len(data) != data[8] + 12:
            raise Exception("Invalid length. Expecting {0}, got {1}".format(
                data[8] + 12, len(data)))
